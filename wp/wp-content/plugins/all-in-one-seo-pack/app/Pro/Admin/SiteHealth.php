...
class SiteHealth extends CommonAdmin\SiteHealth {
	...
	public function testCheckLicense() {
		...
		$label       = sprintf(
			...
			__( 'Your %1$s license key is valid.', 'aioseo-pro' ),
			AIOSEO_PLUGIN_SHORT_NAME
		);
		$description = sprintf(
				// Translators: 1 - The license type.
			__( 'Your license key type for this site is %1$s.', 'aioseo-pro' ),
			'<strong>' . ucfirst( aioseo()->license->getLicenseLevel() ?? __( 'invalid', 'aioseo-pro' ) ) . '</strong>'
		);
