...
class Frontend extends \AIOSEO\Plugin\Common\Breadcrumbs\Frontend {
	...
	protected function getCrumbTemplate( $crumb ) {
		...
		$type = null;
		if ( 'parent' === $crumb['subType'] ) {
			$type = 'parent';
		} elseif ( ! in_array( $crumb['type'], [ 'homePage', 'prefix', 'wcShop', 'blog' ], true ) ) {
			$type = 'current';
		}
		...
		$parentTemplateOverride = aioseo()->breadcrumbs->getOverride( 'parentTemplate' );
		if ( 'parent' === $type && ! empty( $parentTemplateOverride ) ) {
			$templateItem['template'] = isset( $parentTemplateOverride['template'] ) ? $parentTemplateOverride['template'] : aioseo()->helpers->decodeHtmlEntities( $parentTemplateOverride );
		}

		$templateOverride = aioseo()->breadcrumbs->getOverride( 'template' );
		if ( 'current' === $type && ! empty( $templateOverride ) ) {
			$templateItem['template'] = isset( $templateOverride['template'] ) ? $templateOverride['template'] : aioseo()->helpers->decodeHtmlEntities( $templateOverride );
		}

		return apply_filters( 'aioseo_breadcrumbs_template', $templateItem );
	}
...
TEMPLATE;
	...
	public function getBreadcrumbs() {
		if ( is_singular() && ! is_admin() ) {
			$post = CommonModels\Post::getPost( get_the_ID() );
			if ( ! empty( $post->breadcrumb_settings ) ) {
				aioseo()->breadcrumbs->setOverride( (array) $post->breadcrumb_settings );
			}
		}

		$isTax = is_category() || is_tag() || is_tax();
		if ( $isTax && ! is_admin() ) {
			$term = Models\Term::getTerm( get_queried_object_id() );
			if ( ! empty( $term->breadcrumb_settings ) ) {
				aioseo()->breadcrumbs->setOverride( (array) $term->breadcrumb_settings );
			}
		}
